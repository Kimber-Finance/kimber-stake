 ...
 * @notice Distributor contract for rewards to the Kimber protocol
...
contract IncentivesController is IIncentivesController, VersionedInitializable, DistributionManager {
    ...
    function getRewardsBalance(address[] calldata assets, address user) external view override returns (uint256) {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (userState[i].stakedByUser, userState[i].totalStaked) = IAToken(assets[i]).getScaledUserBalanceAndSupply(
                user
            );
        }
    ...
    /**
     * @dev Claims reward for an user, on all the assets of the lending pool, kimbermulating the pending rewards
